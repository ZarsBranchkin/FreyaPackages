local Permissions = _G.Freya:GetComponent "Permissions";
local Intents = _G.Freya:GetComponent "Intents"
local Players = game:GetService "Players";

local prefix = ":"

-- Set up the Admin root permission.
Permissions:CreatePermission("Admin")
...
controllerclass.SetPrefix = function(...)
  local fix = extract(...)
  if type(fix) == 'string' then
    prefix = fix
  else
    return error("[Error][Freya Admin] Bad prefix!", 2);
  end
end
